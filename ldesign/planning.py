...
def _plan_fixed_len_ee(radius, total_len, out_position, height, left_width, right_width, tolerance):
    def calc_min_len(n_u, n_b):
        return right_width + out_position + 4 * radius * n_u - (2 - np.pi / 2) * radius * n_b
    ...
    if abs(available_len[-1] - total_len) < tolerance:
        return [complex(0, 0), complex(0, out_position), complex(right_width, out_position)]
    ...
    if abs(available_len[-1] - total_len) < tolerance:
        return [complex(0, 0), complex(0, radius), complex(right_width - radius, radius),
    ...
    while (4 * u_turn_num + 1) * radius <= out_position:
        ...
        if min_len <= total_len <= max_len:
            ...
            points = [complex(0, 0), complex(0, radius), complex(right_width, radius), complex(right_width, 3 * radius)]
            for i in range(u_turn_num - 1):
                width = min(residue_len / 2 + 2 * radius, left_width + right_width)
                points.extend([complex(right_width - width, (4 * i + 3) * radius),
            ...
            points.extend([complex(right_width - width, (4 * u_turn_num - 1) * radius),
...
def _plan_fixed_len_en(radius, total_len, out_position, height, left_width, right_width, tolerance):
    ...
    if out_x >= 2 * radius:
        ...
        if abs(available_len[-1] - total_len) < tolerance:
            return [complex(0, 0), complex(0, radius), complex(right_width - out_position, radius),
    ...
    while (2 * turn_num + 2) * radius <= height:
        ...
        min_len = height + abs(out_x) + extra_len - (2 - np.pi / 2) * (2 * turn_num + 2) * radius
        max_len = height + (turn_num - 1) * (left_width + right_width) + right_width - (2 - np.pi / 2) * (
                2 * turn_num + 2) * radius
        if turn_num % 2 == 1:
        ...
        if min_len <= total_len <= max_len:
            ...
            for i in range(turn_num):
                ...
                if i % 2 == 0:
                    points.extend([complex(right_width - unused_width, (2 * i + 1) * radius),
                                   complex(right_width - unused_width, (2 * i + 3) * radius)])
                else:
                    points.extend([complex(-left_width + unused_width, (2 * i + 1) * radius),
                                   complex(-left_width + unused_width, (2 * i + 3) * radius)])
                residue_len -= 2 * width_diff
            points.extend([complex(out_x, (2 * turn_num + 1) * radius),
                           complex(out_x, height)])
            return points
        turn_num += 1
    raise ValueError(f"Failed. Available length with given arguments: {available_len}, request length: {total_len}")


def plan_fixed_len(radius, total_len, in_direction: Literal['e', 'n', 'w'], out_side: Literal['e', 'n', 'w', 's'],
                   out_position, height, left_width, right_width):
    tolerance = 1e-2
    if radius <= 0:
    ...
    if in_direction == 'e':
        if out_side == 'e':
            return _plan_fixed_len_ee(radius, total_len, out_position, height, left_width, right_width, tolerance)
        if out_side == 'n':
            return _plan_fixed_len_en(radius, total_len, out_position, height, left_width, right_width, tolerance)
