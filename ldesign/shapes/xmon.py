...
class Xmon(elements.Element):
    def __init__(self, args: XmonArgs = None, config: ldesign.config.Config = None):
        super().__init__(config=config)
        if args is None:
            args = XmonArgs()
        self._init_cell(**asdict(args))

    def _init_cell(self, gap, z_gap, arm_len, arm_width, xy_port, r_port, z_inner_pad_width,
                   z_inner_pad_height, z_outer_pad_width, z_outer_pad_height, z_outer_pad_dist):
        ld_inner = self.config.LD_AL_INNER
        ld_outer = self.config.LD_AL_OUTER
        inner = [
            gdstk.cross(1j * (arm_len + arm_width / 2), 2 * arm_len + arm_width, arm_width, **ld_inner),
            gdstk.rectangle(-z_inner_pad_width / 2 - 1j * z_inner_pad_height, z_inner_pad_width / 2 + 0j,
                            **ld_inner),
        ]
        ...
        outer_cut = [
            gdstk.rectangle(-z_outer_pad_dist / 2 - 1j * z_gap,
                            -z_outer_pad_dist / 2 - z_outer_pad_width - 1j * (z_gap - z_outer_pad_height)),
            gdstk.rectangle(z_outer_pad_dist / 2 - 1j * z_gap,
                            z_outer_pad_dist / 2 + z_outer_pad_width - 1j * (z_gap - z_outer_pad_height)),
        ]
...
if __name__ == '__main__':
    ...
    elem = Xmon()
    r_pad = upad.UPad()
    elem.add_element(r_pad, elem.port_r, r_pad.port_u, elements.Transformation(translation=3))
    elem.view()
